...
<div class="cart_totals <?php if ( $woocommerce->customer->has_calculated_shipping() ) echo 'calculated_shipping'; ?>">
	...
	<?php if ( ! $woocommerce->shipping->enabled || $available_methods || ! $woocommerce->customer->get_shipping_country() || ! $woocommerce->customer->has_calculated_shipping() ) : ?>
		...
		<?php if ( $woocommerce->cart->coupons_enabled() ) { ?>
			<div class="thb-cart-coupon">
				<label for="coupon_code"><?php _e( 'Do you have a promo code?', 'woocommerce' ); ?></label>
		...
		<table cellspacing="0">
			<tbody>
				...
				<?php if ( $woocommerce->cart->needs_shipping() && $woocommerce->cart->show_shipping() && ( $available_methods || get_option( 'woocommerce_enable_shipping_calc' ) == 'yes' ) ) : ?>
					...
					<tr class="shipping">
						<th><?php _e( 'Shipping', 'woocommerce' ); ?> <a href="#">calculate</a></th>
		...
		<?php if ( $woocommerce->cart->get_cart_tax() ) : ?>
			...
			<p><small><?php
				...
				$base_country = $woocommerce->countries->countries[ $woocommerce->countries->get_base_country() ];
				$estimated_text = ( $woocommerce->customer->is_customer_outside_base() && ! $woocommerce->customer->has_calculated_shipping() ) ? sprintf( ' ' . __( ' (taxes estimated for %s)', 'woocommerce' ), $woocommerce->countries->estimated_for_prefix() . __( $base_country, 'woocommerce' ) ) : '';
