...
$thb_theme->setConfig('core/addnewenter', thb_array_asum($thb_config, $config));
 ...
 * Page templates
 */
foreach( thb_config('core/addnewenter', 'templates') as $template ) {
	...
	if( !function_exists('thb_add_portfolio_metabox') ) {
		function thb_add_portfolio_metabox() {
			...
			$thb_container = $thb_metabox->createContainer( '', 'portfolio_loop_container' );
				...
				if( thb_is_admin_template( thb_config('core/addnewenter', 'pagination_disabled') ) ) {
			...
			if( thb_check_template_config('core/addnewenter', 'grid_templates') ) {
				...
				$thb_field->setLabel( thb_config('core/addnewenter', 'grid_templates_columns_label') );
					$thb_field->setOptions( thb_config('core/addnewenter', 'grid_templates_columns') );
				$thb_container->addField($thb_field);

				$grid_image_sizes = thb_config('core/addnewenter', 'grid_image_sizes');
		...
		if( thb_check_template_config('core/addnewenter', 'templates') ) {
 ...
 * -----------------------------------------------------------------------------
 */
if( thb_config('core/addnewenter', 'options') ) {
	$thb_page = $thb_theme->getAdmin()->getMainPage();

	$thb_tab = new THB_Tab( __('addnewenter', 'thb_text_domain'), 'addnewenter' );
		...
		if( thb_config('core/addnewenter', 'works_navigation') ) {
 ...
 * -----------------------------------------------------------------------------
 */
if( thb_config('core/addnewenter', 'single') ) {
...
if( !function_exists('thb_portfolio_script_and_styles') ) {
	function thb_portfolio_script_and_styles() {
		...
		$thb_frontend->addScript(thb_get_module_url('core/addnewenter') . '/js/thb.portfolio.js', $conds);
		$thb_frontend->addScript(thb_get_module_url('core/addnewenter') . '/js/config.js', $conds);
		$thb_frontend->addStyle(THB_FRONTEND_CSS_URL . '/isotope.css', $conds);
	}

	if( thb_config('core/addnewenter', 'isotope') ) {
...
if( !function_exists('thb_portfolio_filter') ) {
	function thb_portfolio_filter( $params=array() ) {
		...
		thb_get_module_template_part('core/addnewenter', $params['portfolio_filter_template'], $params);
	}
}
...
if( !function_exists('thb_portfolio_loop') ) {
	function thb_portfolio_loop( $params=array() ) {
		...
		$default = array(
			'portfolio_template'      => 'addnewenter',
			'portfolio_item_template' => 'addnewenter-item',
			'num_cols'                => thb_get_post_meta($thb_page_id, 'portfolio_columns'),
			'work_image_size'         => thb_portfolio_get_image_size()
		);
		$params = thb_array_asum($default, $params);

		thb_get_module_template_part('core/addnewenter', $params['portfolio_template'], $params);
	}
}
...
if( ! function_exists('thb_portfolio_get_image_size') ) {
	function thb_portfolio_get_image_size() {
		$thb_page_id = thb_get_page_ID();
		$image_sizes = thb_config('core/addnewenter', 'grid_image_sizes');
...
if( !function_exists('thb_portfolio_item') ) {
	function thb_portfolio_item( $params=array() ) {
		...
		thb_get_module_template_part('core/addnewenter', $params['portfolio_item_template'], $params);
	}
}