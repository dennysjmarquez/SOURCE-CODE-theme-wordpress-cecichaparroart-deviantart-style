...
if( !class_exists('THB_Admin') ) {
	class THB_Admin {
		...
		/**
		 * Create the duplicable table structure.
		 *
		 * @return void
		 */
		private function createDuplicableTable()
		{
			global $wpdb;
			include_once(ABSPATH . 'wp-admin/includes/upgrade.php');

			$sql = "CREATE TABLE " . $wpdb->prefix . THB_DUPLICABLE_TABLE . " (
		...
		private function _loadInstallationDetails()
		{
			...
			if( !isset($this->_installationDetails['duplicable_table']) || $this->_installationDetails['duplicable_table'] == '0' || $this->hasJustBeenActivated() ) {
				$this->createDuplicableTable();
				$this->_installationDetails['duplicable_table'] = '1';
			}
