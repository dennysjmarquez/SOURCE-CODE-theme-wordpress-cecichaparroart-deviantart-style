...
if( !function_exists('thb_title') ) {
	function thb_title( $sep=' | ', $echo=true ) {
		...
		elseif (is_tag()) {
			...
			$title = 'Browsing tag '.single_cat_title( '', false ).' on '. $name;
		}
		elseif (!is_tag() && the_category2() && !is_single()) {
			
			$categories = get_the_category();
			$cat = $categories[0]->slug;
			
			$title = 'Browsing '.$cat.' on '. $name;
		...
		if( !empty($_POST) && isset($_POST[$key]) ) {
			...
			for( $i=0; $i<$post_count; $i++ ) {
				...
				thb_duplicable_add(array(
					'ord'       => $i,
					'field_key' => $key,
					'value'     => serialize( array('family' => $family, 'css' => $font['css'][$i], 'variants' => implode(',', $font['variants'][$i]), 'folder' => $font['folder'][$i]) ),
					'meta'      => serialize( $meta ),
					'post_id'   => 0
				));
			}
